...
由于 WSL 不支持 Windows 的输入法，我们需要在 Linux 内部安装 Fcitx5。
...
```bash
# 将配置写入 ~/.bashrc
cat <<EOF >> ~/.bashrc# --- Fcitx5 Input Method Configuration for WSLg ---export GTK_IM_MODULE=fcitxexport QT_IM_MODULE=fcitxexport XMODIFIERS=@im=fcitxexport INPUT_METHOD=fcitxexport FCITX_NO_WAYLAND=1  # 强制禁用 Wayland 模块，解决启动崩溃问题#
```
